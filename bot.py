...
async def get_air_quality():
    ...
    try:
        ...
        # ВРЕМЕННЫЙ ОТЛАДОЧНЫЙ ВЫВОД: фрагмент вокруг блока с данными
        idx = text.find("Источник данных")
        if idx != -1:
            start = max(0, idx - 200)
            end = min(len(text), idx + 600)
            print("===== IQAir data snippet =====")
            print(text[start:end])
            print("===== END IQAir data snippet =====")
        else:
        ...
        if aqi is None or not level:
            match = re.search(
                r"\b(\d{1,3})\b[^\d]{0,40}"
                r"(Хорошо|Средне|Вредно для уязвимых групп|Нездорово для чувствительных групп|Нездорово|Очень нездорово|Опасно)",
                text,
            )
            if match:
